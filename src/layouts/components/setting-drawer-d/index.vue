...
<template>
  ...
  >
    <CloseOutlined
      v-if="open" :class="`${prefixCls}-handle-icon${theme === 'light' ? '' : '-dark'}`"
      style="font-size: 20px"
    />
    <SettingOutlined
      v-else :class="`${prefixCls}-handle-icon${theme === 'light' ? '' : '-dark'}`"
      style="font-size: 20px"
    />
  </div>
  <a-drawer :open="open" :width="300" placement="right" :closable="false" @update:open="handleVisible">
    <template #handle>
      ...
      >
        <CloseOutlined
          v-if="open" :class="`${prefixCls}-handle-icon${props.theme === 'light' ? '' : '-dark'}`"
          style="font-size: 20px"
        />
        <SettingOutlined
          v-else :class="`${prefixCls}-handle-icon${props.theme === 'light' ? '' : '-dark'}`"
          style="font-size: 20px"
        />
      </div>
    </template>
    <div :class="`${prefixCls}-content`">
      <Body title="整体风格设计">
        <div :class="`${prefixCls}-block-checkbox`">
          <BlockCheckbox
            :checked="props.theme === 'light'" theme="light" :is-dark="props.theme === 'dark'"
            @click="changeTheme('light')"
          />
          <BlockCheckbox
            v-if="layout !== 'mix'" :checked="props.theme === 'inverted'" theme="inverted"
            :is-dark="props.theme === 'dark'" @click="changeTheme('inverted')"
          />
          <BlockCheckbox
            :checked="props.theme === 'dark'" theme="dark" :is-dark="props.theme === 'dark'"
      ...
      <Body title="导航模式">
        <div :class="`${prefixCls}-block-checkbox`">
          <BlockCheckbox
            theme="side" :checked="layout === 'side'" :is-dark="props.theme === 'dark'"
            @click="changeLayout('side')"
          />
          <BlockCheckbox
            theme="top" :checked="layout === 'top'" :is-dark="props.theme === 'dark'"
            @click="changeLayout('top')"
          />
          <BlockCheckbox
            theme="mix" :checked="layout === 'mix'" :is-dark="props.theme === 'dark'"
            @click="changeLayout('mix')"
          />
        </div>
      </Body>
      <LayoutSetting
        :layout="layout" :content-width="contentWidth" :fixed-header="fixedHeader"
        :fixed-sider="fixedSider" :split-menus="splitMenus" :keep-alive="keepAlive" :accordion-mode="accordionMode"
        :left-collapsed="leftCollapsed" :compact-algorithm="compactAlgorithm" @change-setting="changeSettingLayout"
      />
      <a-divider />

      <Body title="内容区域">
        <RegionalSetting
          :layout="layout" :header="header" :watermark="watermark" :menu-header="menuHeader"
          :footer="footer" :animation-name="animationName" :animation-name-list="animationNameList"
      ...
      <Body title="其他设置">
        <OtherSetting :color-weak="colorWeak" :color-gray="colorGray" @change-setting="changeSettingLayout" />
      </Body>
      <a-divider />
      ...
      <Body>
        <div flex gap-2 flex-col>
          <a-alert
            type="warning" show-icon message="配置栏只在开发环境用于预览，生产环境不会展现，请拷贝后手动修改配置文件"
          >
            <template #icon>
